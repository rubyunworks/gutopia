...
favoriteFruitlbl = GUtopIa::Label.new.text('Favorite Fruit')
   ...
   favoriteFruit = GUtopIa::Text.new.columns(25).lines(4).readonly(false).value('Banana')
     ...
     fruit_panel = GUtopIa::Panel.new.layout(:grid)
     fruit_panel.body = [ [ firstNamelbl,     firstName     ],
                          [ lastNamelbl,      lastName      ],
                          [ favoriteFruitlbl, favoriteFruit ],
                          [ submitbtn,        clearbtn      ] ]
  
      mainWindow = GUtopIa::Window.new.caption('My Cool Window').icon('myIcon.ico')
      mainWindow.layout(:grid)
      mainWindow.body = [ [ menu        ], 
                          [ fruit_panel ] ]
  
        aboutlbl = GUtopIa::Label.new
        aboutlbl.text = "This is a very cool application\n" \
                        "that I created using <b>GUtopIa</b>"
  
        closebtn = GUtopIa::Button.new.text('Close')
  
     aboutWindow = GUtopIa::Window.new.caption('About My Cool App')
     aboutWindow.layout(:grid).body( [ [ aboutlbl ],
                                       [ closebtn ] ] )
